...
interface Props { tx: Transaction; categories: Category[]; budgets: BudgetOption[]; }
...
export default function ReviewCard({ tx, categories, budgets }: Props) {
  ...
  async function retryOcr() {
    setRetrying(true);
    await fetch(`/api/transactions/${tx.id}/retry-ocr`, { method: "POST" });
    router.refresh();
    setRetrying(false);
  }
    ...
    <div className="rounded-2xl bg-white border border-gray-200 shadow-sm overflow-hidden">
      {tx.receipt_url && (
        <a href={tx.receipt_url} target="_blank" rel="noopener noreferrer">
          ...
          <img src={tx.receipt_url} alt="Receipt" className="w-full max-h-52 object-cover object-top" />
        </a>
      )}

      <div className="p-4 space-y-4">
        <div className="flex items-center gap-2">
          {tx.status === "pending_ocr" && (
          ...
          <span className="text-xs text-gray-400 capitalize ml-auto">{tx.source}</span>
        </div>

        {!tx.receipt_url && tx.memo && (
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <input value={payeeName} onChange={(e) => setPayeeName(e.target.value)} placeholder="Business name" className={inputCls} />
          </div>
          <div>
        ...
        <div className="flex gap-2 pt-1">
          {tx.status === "pending_ocr" && tx.receipt_url && (
