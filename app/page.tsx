    ...
    <div className="flex min-h-screen bg-gray-50 text-gray-900">
      ...
      <div className="flex-1 md:pl-56 min-w-0 flex flex-col">
        ...
        {budgetSummaries.length > 0 && (
          <div className={`${bannerBg} px-6 md:px-8 lg:px-10 py-6 md:py-7 flex items-center justify-between gap-4`}>
            <div>
              <p className="text-white/70 text-xs font-medium uppercase tracking-wide mb-1">{monthName} {now.getFullYear()}</p>
              ...
              <p className="text-white/80 text-sm mt-2">{bannerLabel}</p>
            </div>
            <div className="hidden sm:flex flex-col items-end gap-1 text-right shrink-0">
              <div className="text-white/60 text-xs">
                <span className="font-medium text-white">{formatCurrency(totalSpent, "JMD")}</span> of {formatCurrency(totalBudget, "JMD")} spent
              </div>
        ...
        {/* ── Two-column body ───────────────────────────────────────────── */}
        <div className="flex-1 px-4 md:px-8 lg:px-10 py-6 pb-24 md:pb-10">
            ...
            {budgetSummaries.length > 0 && (
              <aside className="lg:w-64 xl:w-72 shrink-0 space-y-4">
                ...
                <div className="rounded-2xl bg-white border border-gray-200 shadow-sm p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-4">{monthName}&apos;s Summary</h3>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-500">Assigned in {monthName}</span>
                    ...
                    <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
                      ...
                      <span className={`text-sm font-bold tabular-nums ${isOver ? "text-red-600" : "text-emerald-600"}`}>
                        {isOver ? "−" : ""}{formatCurrency(Math.abs(totalRemaining), "JMD")}
                      </span>
                    </div>
                  </div>
                ...
                {/* Budget health */}
                <div className="rounded-2xl bg-white border border-gray-200 shadow-sm p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-3">Budget Health</h3>
                  <div className="h-2.5 rounded-full bg-gray-100 mb-3 overflow-hidden">
                    <div
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-400">
                    ...
                    <span>{budgetSummaries.length} {budgetSummaries.length === 1 ? "budget" : "budgets"}</span>
                  </div>
                </div>

                {/* Quick links */}
                <div className="rounded-2xl bg-white border border-gray-200 shadow-sm divide-y divide-gray-100 overflow-hidden">
                  <Link href="/budgets"
                  ...
                  {pendingCount > 0 && (
                    <Link href="/review"
                      className="flex items-center justify-between px-4 py-3 text-sm text-blue-600 hover:bg-blue-50 transition-colors">
